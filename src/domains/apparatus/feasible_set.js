...
function faceAxisAtCameraHeight(face, camera) {
  return [face[0], face[1], camera[2]];
}
...
export class FeasibleSet {
  evaluate({face,camera,mirrorCentre,mirrorNormal,shoulder,handedness="right",r=HEAD_RADIUS_M}) {
    ...
    // v5 §4 defines e as camera-to-face-axis offset, while §5 explicitly states that
    // height cannot buy direct/reflected-face clearance. Use the closest point on the
    // vertical face axis at camera height for the plan-view apparatus quantities.
    const F=faceAxisAtCameraHeight(face,camera);
    const n=normalize(mirrorNormal),m=Math.abs(dot(sub(F,mirrorCentre),n)),c=Math.abs(dot(sub(camera,mirrorCentre),n)),u=c-m,a=distance(camera,F),along=dot(sub(camera,F),n),eVec=sub(sub(camera,F),scale(n,along)),e=length(eVec);
    const faceR=reflectPoint(F,mirrorCentre,n),R=a>1e-9?distance(camera,faceR)/a:Infinity,toFace=sub(F,camera),toFaceR=sub(faceR,camera),denom=length(toFace)*length(toFaceR),cosSig=denom>1e-12?Math.min(1,Math.max(-1,dot(toFace,toFaceR)/denom)):1,sigma=Math.acos(cosSig),asinArg=a>1e-9?Math.min(1,r/a):1,clearance=sigma-Math.asin(asinArg);
    ...
    const shoulderLateral=shoulder?sub(sub(shoulder,F),scale(n,dot(sub(shoulder,F),n))):null,crossed=!!(shoulderLateral&&e>1e-9&&dot(eVec,shoulderLateral)<0),sameSideRequired=a>A_CROSS_BODY_M,sameSideOk=!sameSideRequired||!crossed,sameSideMargin=sameSideOk?Math.abs(A_CROSS_BODY_M-a):A_CROSS_BODY_M-a;
    const boundaries=[
    ...
    return{m,u,c,a,e,R,sigma,clearance,r,r_epistemic:DEC["DEC-R"].epistemic_status,face_axis_point:F,eclipseLimit,eMin,eMax:E_ABDUCTION_CEILING_M,crossed,sameSideRequired,handedness,inside:reasons.length===0,reasons,boundaries,distance_to_boundary:bindingRow?.distance??-Infinity,binding:bindingRow?.id||null};
  }
  lateralDir(face,camera,n){const F=faceAxisAtCameraHeight(face,camera),along=dot(sub(camera,F),n);let lat=sub(sub(camera,F),scale(n,along));if(length(lat)<1e-6)lat=Math.abs(n[2])<.9?cross(n,[0,0,1]):cross(n,[1,0,0]);return normalize(lat);}
  project(translation,d_M,face,camera,n,row){if(row.inside)return{translation:translation.slice(),d_M,moved:false};let nextT=translation.slice();const F=faceAxisAtCameraHeight(face,camera),lat=this.lateralDir(face,camera,n);if(row.e<row.eMin)nextT=add(nextT,scale(lat,row.eMin-row.e+.002));if(row.e>E_ABDUCTION_CEILING_M)nextT=add(nextT,scale(lat,-(row.e-E_ABDUCTION_CEILING_M+.002)));const radial=sub(camera,F);if(length(radial)>1e-9){const dir=normalize(radial),targetA=Math.min(A_SAME_SIDE_LIMIT_M-.002,Math.max(A_ELBOW_IN_M+.002,row.a));if(Math.abs(targetA-row.a)>1e-9)nextT=add(nextT,scale(dir,targetA-row.a));}if(row.sameSideRequired&&row.crossed)nextT=add(nextT,scale(lat,.02));return{translation:nextT,d_M,moved:true};}
