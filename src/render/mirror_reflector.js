...
import { apertureCorners } from "../domains/mirror/mesh.js";
import { finiteApertureTest } from "../domains/reflection/reflect.js";
...
export class MirrorReflector {
  ...
  ensureFurniture(THREE) {
    if (this.frame) return;
    this.frame = new THREE.Mesh(
      new THREE.BoxGeometry(1, 1, 0.02),
      new THREE.MeshStandardMaterial({ color: 0x2a3036, metalness: 0.4, roughness: 0.55, clippingPlanes: this.clipPlanes }),
    );
    this.contact = new THREE.Mesh(
      new THREE.BoxGeometry(0.08, 0.08, 1),
      new THREE.MeshStandardMaterial({ color: 0x3a4046, roughness: 0.8 }),
    );
  ...
  insideClip(worldPoint) {
    const THREE = this.THREE;
    const v = new THREE.Vector3(...worldPoint);
    if (!this.clipPlanes.length) return false;
    return this.clipPlanes.every((p) => p.distanceToPoint(v) >= 0);
  }

  clipAgreesWithAperture(worldPoint, effective, eps = 1e-4) {
    const vis = finiteApertureTest(worldPoint, effective.camera.world.translation, effective.mirror);
    const clip = this.insideClip(worldPoint);
    return vis.visible === clip || Math.abs(Number(vis.visible) - Number(clip)) === 0;
  }
  ...
  update(effective, sources) {
    ...
    const w = effective.mirror.width_m;
    const h = effective.mirror.height_m;
    const basis = effective.mirror.basis;
    this.frame.position.set(...M);
    ...
    this.frame.scale.set(w + 0.04, h + 0.04, 1);
    const bottom = [...M];
    bottom[2] = Math.min(M[2] - h / 2, 0);
    this.contact.position.set(M[0], M[1], (M[2] - h / 2) * 0.5);
    this.contact.scale.set(1, 1, Math.max(0.02, M[2] - h / 2));
    this.contact.visible = M[2] - h / 2 > 0.02;
  }
}
