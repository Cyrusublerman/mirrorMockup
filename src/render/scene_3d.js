...
export async function createScene3D(canvas,app,opts={}){
  ...
  return {sync,resize,renderer,outputRenderer,scene,camera:editorCam,capture,reflector,hitTest,orbit:orbitBy,dolly,setEditorView,swapInset,dragDeltaWorld,setBodyMode:(kind)=>{if(!["GESTURE","VOLUME","CONTOUR"].includes(kind))throw new Error(`unknown representation ${kind}`);bodyMode.kind=kind;},setFrameScope:(id)=>{if(!["BODY","APPARATUS"].includes(id))throw new Error(`unknown frame scope ${id}`);frameScope.id=id;frame.userScale=1;},viewState,viewLabel,workspace:{get editor_view(){return viewState.main_pane==="CAPTURE"?"CAMERA":viewState.editor_view;},set editor_view(v){setEditorView(v);}},SEMANTIC,boneIndex};
}
