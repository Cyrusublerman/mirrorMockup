...
export async function createScene3D(canvas,app,opts={}){
  ...
  const pickMats={idle:new THREE.MeshBasicMaterial({color:0xc41e63,transparent:true,opacity:.18,depthTest:true}),hot:new THREE.MeshBasicMaterial({color:0xc41e63,transparent:true,opacity:0,depthTest:true})};
  ...
  const bodyMode={kind:"GESTURE"};const orbit={theta:.7,phi:1.15};const room={id:"POSE"};const frame={target:[0,.9,.9],radius:2.4,userScale:1};const raycaster=new THREE.Raycaster();const ndc=new THREE.Vector2();let paneW=800,paneH=600;
