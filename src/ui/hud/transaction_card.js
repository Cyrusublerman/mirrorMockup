export class TransactionCard {
  mount(el, proj, handlers = {}) {
    ...
    const comp = proj.compensation;
    const fea = proj.feasible;
    const intent = proj.occlusion_intent;
    const show = !!(le || (comp && Math.abs(Number(comp.to) - Number(comp.from)) > 0.01) || proj.effective?.transaction === "PROJECTED" || proj.effective?.transaction === "FAIL");
    if (!show) {
      el.hidden = true;
      return;
    }
    ...
    if (comp && Math.abs(Number(comp.to) - Number(comp.from)) > 0.01) {
      const row = document.createElement("div");
      row.className = "mp-row";
      row.append(
        chip("KEEP " + fmt(comp.to), handlers.accept),
        chip("RELEASE LOCK", handlers.release),
...
function resultLine(comp, proj) {
  if (comp && Number.isFinite(comp.from) && Number.isFinite(comp.to)) {
    return `requested ${fmt(comp.from)} m → effective ${fmt(comp.to)} m`;
  }
